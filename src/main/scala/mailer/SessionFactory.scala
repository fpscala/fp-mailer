...
trait SessionFactory {
  ...
  /** Adds the given ''JavaMail'' property to the set of existing properties (used usually as the prepend operator).
    *
    * @param p
    *   property to add
    * @return
  ...
  /** Adds the given ''JavaMail'' property to the set of existing properties (used usually as the append operator).
    *
    * @param p
    *   property to add
    * @return
  ...
  /** Returns the created ''JavaMail'' session, optionally credentials can be provided if required.
    *
    * @param credentials
    *   credentials, represented as the pair of (username, password), optional
    * @return
    *   Plain mail Session
    */
  ...
  def apply(): SessionFactory = new SessionFactory {
    override def append(p: Prop): SessionFactory = {
      ...
      case class impl(prev: impl, prop: Prop) extends SessionFactory {
        override def append(p: Prop): SessionFactory = impl(this, p)
        ...
        override def session(credentials: Option[(String, String)] = None): Session = {
          val ps = properties()
          credentials match {
            case None => Session.getInstance(ps)
            case Some((u, p)) =>
